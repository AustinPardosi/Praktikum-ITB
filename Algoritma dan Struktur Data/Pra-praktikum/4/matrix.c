...
/* Implementasi ABSTRACT DATA TYPE LIST MATRIX */
...
void displayMatrix(Matrix m){
    for (int i = 0; i <= getLastIdxRow(m); i++) {
        for (int j = 0; j <= getLastIdxCol(m)-1; j++) {
            printf("%d ", ELMT(m, i, j));
        }
...
boolean isMatrixNotEqual(Matrix m1, Matrix m2){
    ...
    if ((ROW_EFF(m1) != ROW_EFF(m2)) || (COL_EFF(m1) != COL_EFF(m2))) {
        ...
        while ((i <= getLastIdxRow(m1)) && stillEqual) {
            while ((j <= getLastIdxCol(m1)) && stillEqual) {
                if (ELMT(m1, i, j) != ELMT(m2, i, j)) {
                    stillEqual = false;
                }
                else {
...
boolean isSparse(Matrix m){
    ...
    persentage = countNot0 / countElement; 
...
/* Menghitung nilai determinan m */
    ...
    if (ROW_EFF(m) == 1) {
        ...
        for (p = 0; p <= getLastIdxCol(m); p++) {
            indexRowMinor = 0;
            for (int i = 1; i <= getLastIdxRow(m); i++) {
                indexColMinor = 0;
                for (int j = 0; j <= getLastIdxRow(m); j++) {
                    if (j != p) {
                        ELMT(minor, indexRowMinor, indexColMinor) = ELMT(m, i, j);
                        indexColMinor++;
                    }
                }
                indexRowMinor++;
            ...
            if (p % 2 == 0) {
                det = det + (ELMT(m, 0, p)*determinant(minor));
            }
            else {
                det = det - (ELMT(m, 0, p)*determinant(minor));
            }
